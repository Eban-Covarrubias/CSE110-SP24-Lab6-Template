describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      if (itemData.title.length === 0 || itemData.price.length === 0 || itemData.image.length === 0) {
          ...
          break; // Stop checking if any item is not populated
      }
      expect(allArePopulated).toBe(true);
  }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Access the shadowRoot of the element and query the button from it
    const buttonTextAfterClick = await productItem.evaluate(productItem => {
      const shadowRoot = productItem.shadowRoot;
    ...
    expect(buttonTextAfterClick.trim()).toBe("Remove from Cart");
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const productItem of productItems) {
        // Check if shadowRoot is a function before calling it
        if (typeof productItem.shadowRoot === 'function') {
            const shadowRoot = await productItem.shadowRoot();
            const button = await shadowRoot.$('button');
            await button.click();
        } else {
            console.log('ShadowRoot method not available for product item.');
        }
    }
